...
class User extends CI_Controller {
   ...
   */
    ...
    $data['users'] = $this->User_model->allClients();
    ...
    if($user){
      ...
      if($user->role == "Administrador"){
        ...
        $data['ventas'] = $this->Buy_model->getSales();
        $this->load->view('user/list',$data);
      }else{
        $this->load->model('Categories_model');
        $this->load->model('Buy_model');
        $result = $this->Categories_model->getFathers();
        $buys = $this->Buy_model->getUserBuys($user->id);
        if($result){
  ...
  public function create(){
    ...
    $user = ['name' => $name, 'seconName' => $secondName, 'password' => $password, 'role' => 'Cliente'];
